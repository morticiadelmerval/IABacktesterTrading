...
OUTPUT_FILE = "data/tspulse_signals.json"
...
def run_walk_forward_training():
    ...
    for tk in TICKERS:
        ...
        samples_past = []
        samples_future = []
        for i in range(len(returns) - context_len - forecast_len + 1):
            samples_past.append(returns[i : i + context_len])
            samples_future.append(returns[i + context_len : i + context_len + forecast_len])
            
        samples_past = np.array(samples_past)
        samples_future = np.array(samples_future)
        
        if len(samples_past) < 100: continue
        print(f"\n[{tk}] Procesando {len(samples_past)} ventanas para Walk-Forward...")
        ...
        for train_end in range(min_train_size, len(samples_past), step_size):
            ...
            class TSTrainDataset(Dataset):
                ...
                def __getitem__(self, idx):
                    return (torch.tensor(self.X[idx], dtype=torch.float32).unsqueeze(-1),
                            torch.tensor(self.y[idx], dtype=torch.float32).unsqueeze(-1))
            
            train_dataset = TSTrainDataset(samples_past[:train_end], samples_future[:train_end])
            train_loader = DataLoader(train_dataset, batch_size=256, shuffle=True)
            
            model = PatchTSMixerForPrediction.from_pretrained(MODEL_NAME)
            model.to(device)
            model.train()
            ...
            model.eval()
            test_dataset = TSPulseDataset(samples_past[train_end:test_end])
            ...
            preds_arr = np.concatenate(preds_list, axis=0) # (samples, 96, 1)
        ...
        live_past = []
        live_dates = []
        max_labeled_i = len(returns) - context_len - forecast_len
        for i in range(max_labeled_i + 1, len(returns) - context_len + 1):
            if i >= 0:
                live_past.append(returns[i : i + context_len])
        ...
        if len(live_past) > 0:
            ...
            if 'model' not in locals():
                ...
                model.to(device)
            model.eval()
            live_dataset = TSPulseDataset(live_past)
    ...
    print("Inferencia OOS guardada exitosamente.")
