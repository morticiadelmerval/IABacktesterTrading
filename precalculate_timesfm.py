...
import numpy as np
import pandas as pd
sys.path.append(r'e:\ProyectosIA\TradingViewPersonalsIndicators')
...
def run_timesfm_precalculation():
    ...
    context_len = 100 # Lookback window
                ...
                if len(inputs) == batch_size or i == len(closes) - forecast_len - 1:
                    ...
                    try:
                        ...
                        if "no kernel image is available for execution on the device" in str(e):
                            ...
                            print("Tu RTX 5070 (Arquitectura Blackwell) requiere CUDA 13.x, pero la versión estable actual de PyTorch no la incluye.")
                    ...
                    for j, pf in enumerate(point_forecasts):
                        ...
                        pred_price = (pred_price_norm * seq_std) + seq_mean
                        current_price = closes[i-batch_size+j if len(inputs) == batch_size else i-len(inputs)+j]
    ...
    with open("timesfm_signals.json", "w") as f:
