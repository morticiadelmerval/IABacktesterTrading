...
STRATEGY_INFO = {
    ...
    "S11": {"type": "REVERSION", "rsi_thresh": 80.0, "mult": 3.1, "name": "Macro + Reversion Optimizada (RSI & BB)"},
}
...
for s_id, params in STRATEGY_INFO.items():
    ...
    elif stype == "REVERSION":
        rsi_th = params["rsi_thresh"]
        mult = params["mult"]
        desc = f"V11: Reversion Optimizada. Vende cuando RSI(14) > {rsi_th} o BB_Upper > {mult}x std. Evita caídas tras extremo sobrecomprado."
        inds = ["SPY Macro Crash Guard", f"RSI 14 > {rsi_th}", f"BB ({mult}x)"]
        pine_active = f"rsi = ta.rsi(close, 14)\nbbUp = ta.sma(close, 20) + {mult:.1f} * ta.stdev(close, 20)\nactiveExit = (rsi > {rsi_th}) or (close > bbUp)"
...
def generate_signals(df, ticker, spy_idx, spy_ret, params, commission=0.0):
    ...
    if stype == "ROC_VOL":
        ...
        bb_up = df['SMA_20'] + params["mult"] * df['STD_20']
        active_exit = (df['RSI_14'] > params["rsi_thresh"]) | (df['Close'] > bb_up)
