...
def generate_signals(df, ticker, spy_idx, spy_ret, params, commission=0.0):
    ...
    if stype == "ROC_VOL":
        ...
        uptrend = (df['Close'] > sma_200) & (macd > signal)
        
        cost_barrier = commission * 2.0
        # Require AI and uptrend
        active_long = uptrend & (preds_aligned > (0.015 + cost_barrier))
        
        ai_wants_out = preds_aligned < -commission
        
        active_exit = (~uptrend) | (ai_wants_out & (preds_aligned < -0.02))
